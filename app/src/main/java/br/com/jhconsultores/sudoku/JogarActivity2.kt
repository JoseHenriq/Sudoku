...
class JogarActivity2 : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Pincéis
        pincelVerde.color     = resources.getColor(R.color.verde)
        pincelBranco.color    = resources.getColor(R.color.white)
        pincelPreto.color     = resources.getColor(R.color.black)
        pincelAzul.color      = resources.getColor(R.color.azul)
        pincelLaranja.color   = resources.getColor(R.color.laranja)
        pincelPurple200.color = resources.getColor(R.color.purple_200)
        ...
        iViewSudokuBoard!!.setOnTouchListener { _, event -> //--- Coordenadas tocadas
            ...
            val strLog = "-> Celula tocada: linha = " + intLinha + ", coluna = " + intCol +
                    ", numero = " + intNum
            Log.d(cTAG, strLog)
            ...
            if (intNum > 0) {
                ...
                intColJogar = 0
                //--------------------------------------------
                mostraNumsIguais(intNum) //intLinha, intCol, intNum);
                //--------------------------------------------
            } else {
                flagJoga = true
        ...
        iViewNumsDisps!!.setOnTouchListener { _, event -> //--- Só transfere o número para o board se estiver jogando
            if (flagJoga) {
                ...
                val strLog = "-> Celula tocada em NumDisp: coluna = " + intCol +
                        ", qtidd = " + intQtidd
                Log.d(cTAG, strLog)
        ...
        btnDraw.setOnClickListener {
            ...
            if (btnDraw.text == strLegTexto) {
                ...
                when (idxPreset) {
                    1 ->                        //--------------------------------------------
                        arArIntNums = copiaArArInt(arArIntPreset1)
                    2 ->                        //--------------------------------------------
                        arArIntNums = copiaArArInt(arArIntPreset2)
                    3 ->                        //--------------------------------------------
                        arArIntNums = copiaArArInt(arArIntPreset3)
                    4 ->                        //--------------------------------------------
                        arArIntNums = copiaArArInt(arArIntPreset4)
                }
                for (intLinha in 0..8) {
                    for (intCol in 0..8) {
                        val intNum = arArIntNums[intLinha][intCol]
    ...
    /*
    //--- desenhaImg
    fun desenhaImg(intImgResource: Int) {

        //myImage = BitmapFactory.decodeResource(getResources(), intImgResource)
        //		.copy(Bitmap.Config.ARGB_8888, true);
        copiaBmpByBuffer(bmpSudokuBoard, myImage)
        ...
        //--- Atualiza imageView do layout
        //-------------------------------------
        iView_SudokuBoard!!.setImageBitmap(myImage)
        //-------------------------------------
    ...
    */
    ...
    private fun escreveCelula(yCell: Int, xCell: Int, strTxt: String, pincel: Paint?) {
        ...
        //----------------------------------------------------------
        val strLog = "-> Coordenadas (Col = " + xCell + ", Linha: " + yCell + ") : (" + xCoord +
                ", " + yCoord + ") = " + strTxt
        Log.d(cTAG, strLog)
    ...
    private fun determinaGrandezasGraficas() {
        ...
        try {
            ...
            val intDyWidth  = mSize.x
            val intDyHeight = mSize.y
            var strLog = "-> Display: Largura: " + intDyWidth + " pixels, Altura  : " +
                    intDyHeight + " pixels"
            Log.d(cTAG, strLog)
